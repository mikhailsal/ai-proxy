...
def iter_json_blocks(f) -> Iterator[Tuple[int, str]]:
...
def parse_log_entry(json_text: str) -> Optional[Dict]:
    try:
...
def normalize_entry(entry: Dict) -> Optional[Dict]:
    from ..utils.datetime_utils import safe_iso_to_datetime
    ...
    if ts_iso is None:
        return None
    ts_iso = str(ts_iso)
    dt_obj = safe_iso_to_datetime(ts_iso)
    if dt_obj is None:
        return None
