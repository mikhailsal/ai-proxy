...
class TestRouter:
    ...
    def test_get_adapter_invalid_provider(self):
        ...
        with patch("ai_proxy.core.routing.settings"):
            ...
            assert "Unsupported provider: invalid_provider" in str(
    ...
    async def test_route_chat_completions_non_streaming(self):
        ...
        with patch("ai_proxy.core.routing.settings") as mock_settings:
            ...
            with patch("ai_proxy.core.routing.logger") as mock_logger:
                ...
                with patch(
                    "ai_proxy.core.routing.get_endpoint_logger"
                ) as mock_endpoint_logger:
                    mock_endpoint_logger.return_value = MagicMock()

                    with patch(
                        "ai_proxy.core.routing.get_model_logger"
                    ) as mock_model_logger:
                        ...
                        result = await test_router.route_chat_completions(
                            request_data, "test_api_key"
                        )
            ...
            with patch("ai_proxy.core.routing.logger") as mock_logger:
                ...
                with patch(
                    ...
                    with patch(
                        ...
                        mock_adapter = AsyncMock()
                        mock_adapter.chat_completions.side_effect = Exception(
                            "Test error"
                        )
                        mock_adapter.get_name = MagicMock(return_value="MockAdapter")
                        test_router.adapters["openrouter"] = mock_adapter
                        ...
                        with pytest.raises(Exception, match="Test error"):
                            await test_router.route_chat_completions(
                                request_data, "test_api_key"
                            )
            ...
            with patch("ai_proxy.core.routing.logger") as mock_logger:
                ...
                with patch(
                    ...
                    with patch(
                        ...
                        assert result == mock_response
                        assert (
                            request_data["model"] == "gemini-1.5-pro"
                        )  # Should be updated with mapped model
