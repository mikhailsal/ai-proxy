...
def test_build_partition_fts_success(temp_db_path):
    ...
    with mock.patch("ai_proxy.logdb.fts._sqlite_supports_fts5", return_value=True):
        ...
        assert (
            indexed == 4
        )  # user test, assistant response, user q (gemini), assistant a
        assert skipped == 1
...
def test_build_partition_fts_large_content_cap(temp_db_path):
    ...
    conn.execute(
        "INSERT INTO requests VALUES (?, ?, ?, ?, ?, ?);",
        ("r1", "", "", "", json.dumps({"messages": [{"content": large}]}), "{}"),
    )
    conn.commit()
    conn.close()
    ...
    with mock.patch("ai_proxy.logdb.fts._sqlite_supports_fts5", return_value=True):
        ...
        content = conn.execute(
            "SELECT content FROM request_text_index WHERE request_id='r1'"
        ).fetchone()[0]
...
def test_build_partition_fts_small_batch_no_mid_insert(temp_db_path):
    ...
    conn.execute(
        "INSERT INTO requests VALUES (?, ?, ?, ?, ?, ?);",
        ("r1", "", "", "", json.dumps({"messages": [{"content": "small"}]}), "{}"),
    )
    conn.commit()
    conn.close()
...
def test_build_partition_fts_large_batch(temp_db_path):
    conn = open_connection_with_pragmas(temp_db_path)
    conn.execute(
        "CREATE TABLE requests (request_id TEXT PRIMARY KEY, endpoint TEXT, model_original TEXT, model_mapped TEXT, request_json TEXT, response_json TEXT);"
    )
    for i in range(501):  # To trigger mid-batch at 500 and final
        req_json = json.dumps({"messages": [{"role": "user", "content": f"test {i}"}]})
        conn.execute(
            "INSERT INTO requests VALUES (?, ?, ?, ?, ?, ?);",
            (f"r{i}", "chat", "model", "mapped", req_json, "{}"),
        )
    conn.commit()
    conn.close()
...
def test_build_fts_for_range_multiple_existing(tmp_path):
    ...
    with (
        mock.patch("ai_proxy.logdb.fts._sqlite_supports_fts5", return_value=True),
        mock.patch(
            "ai_proxy.logdb.fts.build_partition_fts",
            side_effect=lambda p: (5, 1) if p == p1 else (3, 0),
        ),
    ):
...
def test_build_fts_for_range_defaults_to_today():
    ...
    with mock.patch("ai_proxy.logdb.fts.dt.date") as mock_date:
        ...
        with (
            mock.patch("ai_proxy.logdb.fts.os.path.isfile", return_value=False),
...
def test_build_fts_for_range_deduplicates_paths():
    with (
        mock.patch(
            "ai_proxy.logdb.fts.compute_partition_path", return_value="/same/path"
        ),
        mock.patch(
            "ai_proxy.logdb.fts.build_partition_fts", return_value=(1, 0)
        ) as mock_build,
