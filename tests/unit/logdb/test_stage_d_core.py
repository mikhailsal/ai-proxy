...
def test_build_partition_fts_indexes_text(tmp_path):
    # Skip if FTS5 not supported by runtime sqlite
    try:
    ...
    from tests.unit.logdb.test_stage_d_utils import _prepare_db_with_rows

    db_path = _prepare_db_with_rows(tmp_path)
    indexed, skipped = build_partition_fts(db_path)
    assert indexed > 0
    assert skipped == 0
